...
const readline      = require('readline');  // added for confirmation prompts

// ─────────────────────────────────────────────
//  Custom Guard & Confirmation Helpers
// ─────────────────────────────────────────────

/** Read allowed root from ~/.deepseek-agent/config.json */
function getAllowedRoot() {
  const configPath = path.join(process.env.HOME || '', '.deepseek-agent/config.json');
  try {
    const configFile = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    return configFile.ALLOWED_ROOT || null;
  } catch {
    return null; // no restriction
  }
}

/** Check if a file path is inside the allowed root */
function isPathAllowed(filePath) {
  const allowedRoot = getAllowedRoot();
  console.log(`🔍 Checking path: ${filePath}`);
  console.log(`🔍 Allowed root: ${allowedRoot || '(none)'}`);
  if (!allowedRoot) {
    console.log('⚠️ No ALLOWED_ROOT set — allowing all paths.');
    return true;
  }
  const absolutePath = path.resolve(filePath);
  const absoluteRoot = path.resolve(allowedRoot);
  const allowed = absolutePath.startsWith(absoluteRoot);
  console.log(`✅ Allowed? ${allowed}`);
  return allowed;
}

/** Ask for user confirmation before an action */
function askForConfirmation(action, detail) {
  return new Promise((resolve) => {
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    });
    rl.question(`\n⚠️  About to execute: ${action}\n   ${detail}\nConfirm? (y=execute / n=skip / a=abort all): `, (answer) => {
      rl.close();
...
const TOOLS = {
  ...
  read_file: {
    ...
    async execute({ path: filePath, start_line, end_line }) {
      // Guard: read only if inside allowed root
      if (!isPathAllowed(filePath)) {
        throw new Error(`⛔ Access denied: ${filePath} is outside the allowed workspace.`);
      }
  ...
  write_file: {
    ...
    async execute({ path: filePath, content }) {
      console.log(`🔥 write_file called with path: ${filePath}`);

      // Guard: block if outside allowed root
      if (!isPathAllowed(filePath)) {
        throw new Error(`⛔ Access denied: ${filePath} is outside the allowed workspace.`);
      }

      // Confirmation
      const answer = await askForConfirmation('Write file', filePath);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        console.log('⏭️  File write skipped.');
        return '⏭️ File write skipped by user.';
      }
  ...
  append_to_file: {
    ...
    async execute({ path: filePath, content }) {
      console.log(`🔥 append_to_file called with path: ${filePath}`);
      ...
      // Confirmation
      const answer = await askForConfirmation('Append to file', filePath);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        console.log('⏭️  Append skipped.');
        return '⏭️ Append skipped by user.';
      }
  ...
  replace_in_file: {
    ...
    async execute({ path: filePath, find, replace, use_regex = false, all_occurrences = true }) {
      console.log(`🔥 replace_in_file called with path: ${filePath}`);
      ...
      // Confirmation
      const answer = await askForConfirmation('Replace in file', `${filePath} (find: "${find}")`);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        console.log('⏭️  Replace skipped.');
        return '⏭️ Replace skipped by user.';
      }
  ...
  delete_file: {
    ...
    async execute({ path: filePath }) {
      console.log(`🔥 delete_file called with path: ${filePath}`);
      ...
      // Confirmation
      const answer = await askForConfirmation('Delete file', filePath);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        console.log('⏭️  Delete skipped.');
        return '⏭️ Delete skipped by user.';
      }
  ...
  list_directory: {
    ...
    async execute({ path: dirPath = '.', recursive = false, show_hidden = false }) {
      // Guard: directory must be inside allowed root
      if (!isPathAllowed(dirPath)) {
        throw new Error(`⛔ Access denied: ${dirPath} is outside the allowed workspace.`);
      }
  ...
  create_directory: {
    ...
    async execute({ path: dirPath }) {
      console.log(`🔥 create_directory called with path: ${dirPath}`);
      ...
      // Confirmation
      const answer = await askForConfirmation('Create directory', dirPath);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        console.log('⏭️  Directory creation skipped.');
        return '⏭️ Directory creation skipped by user.';
      }
  ...
  move_file: {
    ...
    async execute({ source, destination }) {
      ...
      if (!isPathAllowed(source)) {
        throw new Error(`⛔ Access denied: source ${source} is outside allowed workspace.`);
      }
      if (!isPathAllowed(destination)) {
        throw new Error(`⛔ Access denied: destination ${destination} is outside allowed workspace.`);
      }

      const answer = await askForConfirmation('Move/Rename', `${source} → ${destination}`);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        console.log('⏭️  Move skipped.');
        return '⏭️ Move skipped by user.';
      }
  ...
  copy_file: {
    ...
    async execute({ source, destination }) {
      ...
      const answer = await askForConfirmation('Copy file', `${source} → ${destination}`);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        console.log('⏭️  Copy skipped.');
        return '⏭️ Copy skipped by user.';
      }
  ...
  run_command: {
    ...
    async execute({ command, cwd, timeout = 60_000, env = {} }) {
      console.log(`⚡ run_command called with: ${command}`);

      // Confirmation for command execution
      const answer = await askForConfirmation('Execute command', command);
      if (answer === 'a') {
        console.log('🛑 Session aborted by user.');
        process.exit(0);
      }
      if (answer !== 'y') {
        ...
        return '⏭️ Command skipped by user.';
      }

      // Force cwd to allowed root if set
      const allowedRoot = getAllowedRoot();
      let workDir = cwd ? resolve(cwd) : config.WORKING_DIR;
      if (allowedRoot) {
        // Ensure the working directory is inside the allowed root
        if (!path.resolve(workDir).startsWith(path.resolve(allowedRoot))) {
          console.log(`⚠️  Forcing cwd to allowed root: ${allowedRoot} (requested: ${workDir})`);
          workDir = allowedRoot;
        }
        console.log(`🔒 Running command in: ${workDir}`);
      }
  ...
  find_files: {
    ...
    async execute({ pattern, directory = '.', exclude }) {
      ...
      if (!isPathAllowed(directory)) {
        throw new Error(`⛔ Access denied: ${directory} is outside the allowed workspace.`);
      }
  ...
  write_files: {
    ...
    async execute({ files }) {
      ...
      for (const { path: filePath, content } of files) {
        // Guard each file path
        if (!isPathAllowed(filePath)) {
          throw new Error(`⛔ Access denied: ${filePath} is outside the allowed workspace.`);
        }
        // Ask for confirmation per file (or you could ask once for all)
        const answer = await askForConfirmation('Write batch file', filePath);
        if (answer === 'a') {
          console.log('🛑 Session aborted by user.');
          process.exit(0);
        }
        if (answer !== 'y') {
          ...
          results.push(`⏭️ Skipped: ${filePath}`);
          continue;
        }
        const abs = resolve(filePath);
        fs.mkdirSync(path.dirname(abs), { recursive: true });
      ...
      return `Wrote ${results.filter(r => r.startsWith('✓')).length} files:\n${results.join('\n')}`;
    },
  },
